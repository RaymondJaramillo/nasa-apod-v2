...
public class ApodAdapter extends ArrayAdapter<ApodWithStats> {
  ...
  public ApodAdapter(@NonNull Context context,
      @NonNull List<ApodWithStats> objects, OnClickListener listener) {
  ...
  public View getView(int position, @Nullable View convertView, @NonNull ViewGroup parent) {
    ...
    if (apod.getApod().getMediaType() == MediaType.IMAGE) {
      Picasso.get().load(apod.getApod().getUrl()).into(thumnail);
    } else {
      thumnail.setImageResource(R.drawable.ic_slow_motion);
    }
    thumnail.setContentDescription(apod.getApod().getTitle());
